#!/usr/bin/env python3
r"""
This script will install Ssprompt and its dependencies in an isolated fashion.

It will perform the following steps:
    * Create a new virtual environment using the built-in venv module, or the virtualenv zipapp if venv is unavailable.
      This will be created at a platform-specific path (or `$SSPROMPT_HOME` if `$SSPROMPT_HOME` is set:
        - `~/Library/Application Support/pyssprompt` on macOS
        - `$XDG_DATA_HOME/pyssprompt` on Linux/Unix (`$XDG_DATA_HOME` is `~/.local/share` if unset)
        - `%APPDATA%\pyssprompt` on Windows
    * Update pip inside the virtual environment to avoid bugs in older versions.
    * Install the latest (or a given) version of Ssprompt inside this virtual environment using pip.
    * Install a `ssprompt` script into a platform-specific path (or `$SSPROMPT_HOME/bin` if `$SSPROMPT_HOME` is set):
        - `~/.local/bin` on Unix
        - `%APPDATA%\Python\Scripts` on Windows
    * Attempt to inform the user if they need to add this bin directory to their `$PATH`, as well as how to do so.
    * Upon failure, write an error log to `ssprompt-installer-error-<hash>.log and restore any previous environment.
...
For full documentation, visit https://python-ssprompt.org/docs/#installation.
"""  # noqa: E501
import sys
...
if sys.version_info < (3, 6):  # noqa: UP036
    sys.stdout.write("Ssprompt installer requires Python 3.6 or newer to run!\n")
    sys.exit(1)
...
def data_dir() -> Path:
    if os.getenv("SSPROMPT_HOME"):
        return Path(os.getenv("SSPROMPT_HOME")).expanduser()
    ...
    return base_dir / "pyssprompt"


def bin_dir() -> Path:
    if os.getenv("SSPROMPT_HOME"):
        return Path(os.getenv("SSPROMPT_HOME")).expanduser() / "bin"
...
PRE_MESSAGE = """# Welcome to {ssprompt}!

This will download and install the latest version of {ssprompt},
a dependency and package manager for Python.

It will add the `ssprompt` command to {ssprompt}'s bin directory, located at:
...
POST_MESSAGE = """{ssprompt} ({version}) is installed now. Great!
...
POST_MESSAGE_NOT_IN_PATH = """{ssprompt} ({version}) is installed now. Great!

To get started you need {ssprompt}'s bin directory ({ssprompt_home_bin}) in your `PATH`
environment variable.
{configure_message}
Alternatively, you can call {ssprompt} explicitly with `{ssprompt_executable}`.
...
Add `export PATH="{ssprompt_home_bin}:$PATH"` to your shell configuration file.
"""

POST_MESSAGE_CONFIGURE_FISH = """
You can execute `set -U fish_user_paths {ssprompt_home_bin} $fish_user_paths`
"""

POST_MESSAGE_CONFIGURE_WINDOWS = """"""


class SspromptInstallationError(RuntimeError):
...
class VirtualEnvironment:
    ...
    def make(cls, target: Path) -> "VirtualEnvironment":
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="ssprompt-installer") as temp_dir:
                virtualenv_pyz = Path(temp_dir) / "virtualenv.pyz"
                request = Request(
                    virtualenv_bootstrap_url, headers={"User-Agent": "Python Ssprompt"}
                )
        ...
        # We add a special file so that Ssprompt can detect
        # its own virtual environment
        target.joinpath("ssprompt_env").touch()
    ...
    def run(*args, **kwargs) -> subprocess.CompletedProcess:
        ...
        if completed_process.returncode != 0:
            raise SspromptInstallationError(
...
class Installer:
    METADATA_URL = "https://pypi.org/pypi/ssprompt/json"
    ...
    def install(self, version):
        """
        Installs Ssprompt in $SSPROMPT_HOME.
        """
        self._write(
            "Installing {} ({})".format(
                colorize("info", "Ssprompt"), colorize("info", version)
            )
        )

        with self.make_env(version) as env:
            self.install_ssprompt(version, env)
    ...
    def uninstall(self) -> int:
        if not self.data_dir.exists():
            self._write(
                "{} is not currently installed.".format(colorize("info", "Ssprompt"))
            )
        ...
        if version:
            self._write(
                "Removing {} ({})".format(
                    colorize("info", "Ssprompt"), colorize("b", version)
                )
            )
        else:
            self._write("Removing {}".format(colorize("info", "Ssprompt")))

        shutil.rmtree(str(self.data_dir))
        for script in ["ssprompt", "ssprompt.bat", "ssprompt.exe"]:
            if self.bin_dir.joinpath(script).is_symlink():
    ...
    def make_bin(self, version: str, env: VirtualEnvironment) -> None:
        ...
        script = "ssprompt.exe" if WINDOWS else "ssprompt"
    ...
    def install_ssprompt(self, version: str, env: VirtualEnvironment) -> None:
        self._install_comment(version, "Installing Ssprompt")
        ...
        if self._git:
            ...
            specification = f"ssprompt=={version}"

        env.pip("install", specification)

    def display_pre_message(self) -> None:
        kwargs = {
            "ssprompt": colorize("info", "Ssprompt"),
            "ssprompt_home_bin": colorize("comment", self.bin_dir),
        }
    ...
    def display_post_message_windows(self, version: str) -> None:
        ...
        self._write(
            message.format(
                ...
                ssprompt_home_bin=colorize("comment", self.bin_dir),
                ssprompt_executable=colorize("b", self.bin_dir.joinpath("ssprompt")),
                configure_message=POST_MESSAGE_CONFIGURE_WINDOWS.format(
                    ssprompt_home_bin=colorize("comment", self.bin_dir)
                ),
                test_command=colorize("b", "ssprompt --version"),
            )
        )
    ...
    def get_version(self):
        ...
        self._write(colorize("info", "Retrieving Ssprompt metadata"))
    ...
    def _get(self, url):
        request = Request(url, headers={"User-Agent": "Python Ssprompt"})
...
def main():
    parser = argparse.ArgumentParser(
        description="Installs the latest (or given) version of ssprompt"
    )
    parser.add_argument(
        "-p",
        "--preview",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "fetching the latest version of Ssprompt available online."
        ),
    )
    parser.add_argument(
        "--git",
    ...
    installer = Installer(
        version=args.version or os.getenv("SSPROMPT_VERSION"),
        preview=args.preview or string_to_bool(os.getenv("SSPROMPT_PREVIEW", "0")),
        force=args.force,
        accept_all=args.accept_all
        or string_to_bool(os.getenv("SSPROMPT_ACCEPT", "0"))
        or not is_interactive(),
        path=args.path,
        git=args.git,
    )

    if args.uninstall or string_to_bool(os.getenv("SSPROMPT_UNINSTALL", "0")):
        return installer.uninstall()

    try:
        return installer.run()
    except SspromptInstallationError as e:
        installer._write(colorize("error", "Ssprompt installation failed."))
        ...
        if e.log is not None:
            ...
            _, path = tempfile.mkstemp(
                suffix=".log",
                prefix="ssprompt-installer-error-",
                dir=str(Path.cwd()),
                text=True,
            )
